...
def kernel_density(network, events, bandwidth, orig_nodes, kernel='quadratic'):
    ...
    def compute_split_multiplier(prev_D, n):
        ...
        --20130407 wdnanji
        A problem here is that it may run forever, the reason is that two points in the 
        path may have same coordinates. So we have to differentiate points by its id rather
        than geometry
